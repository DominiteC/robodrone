...
* @author Sevenfite
* @date 2023/12/10
* @brief  本文件是USMART的移植文件，主要修改了以下内容：
* 修改说明：
* 1，不使用原本的串口接收缓冲区，改为使用串口数据结构体中的接收缓冲区，
*    注意：串口数据结构体要在主函数中先自行初始化，如果使用DMA接收，则发送不需要以换行符结尾，如果使用中断接收，则发送需要以换行符结尾
* 2，不使用原本的定时器初始化，自己实现了定时扫描，使用的方法是通过定时器观察者模式，定时扫描
* 3，由于使用了自己的定时器，所以不能使用函数运行时间计算功能
* 4，输出使用的printf，要确保printf能用，输出方式可以在usmart_port.h中修改
*/
...
#endif
#if USMART_TIME_OBSERVER_SEVENFITE == 1
...
uint8_t usmart_sys_cmd_exe(char *str)
{
    ...
    switch (i)
    {
        ...
        case 1: /* 帮助指令 */
            ...
            USMART_PRINTF("或函数入口地址等作为参数),单个函数最多支持10个输入参数,并支持\r\n"),
            ...
            USMART_PRINTF("list:   可用函数列表\r\n");
            USMART_PRINTF("id:     可用的函数的ID列表\r\n");
            USMART_PRINTF("hex:    参数16进制显示,后跟空格+数字即执行进制转换哦\r\n");
            USMART_PRINTF("dec:    参数10进制显示,后跟空格+数字即执行进制转换哦\r\n");
...
#if USMART_TIME_OBSERVER_SEVENFITE == 1
Observer usmartObserver={9,usmart_scan};//定义观察者
#endif
/**
 * @brief       初始化USMART
...
#if USMART_TIME_OBSERVER_SEVENFITE == 1   
    gl_TIM6_IT.add(&gl_TIM6_IT,&usmartObserver);//订阅定时器中断
#endif
}
...
void usmart_exe(void)
{
    ...
    switch (usmart_dev.pnum)
    {
        ...
        case 1: /* 有1个参数 */
            ...
            //实际上写 res = (*(uint32_t(*)())usmart_dev.funs[id].func)(temp[0]); 也可以，但是会报警告,下同
            break;
        ...
        case 3: /* 有3个参数 */
            res = (*(uint32_t(*)(uint32_t,uint32_t,uint32_t))usmart_dev.funs[id].func)(temp[0], temp[1], temp[2]);
            break;

        case 4: /* 有4个参数 */
            res = (*(uint32_t(*)(uint32_t,uint32_t,uint32_t,uint32_t))usmart_dev.funs[id].func)(temp[0], temp[1], temp[2], temp[3]);
            break;

        case 5: /* 有5个参数 */
            res = (*(uint32_t(*)(uint32_t,uint32_t,uint32_t,uint32_t,uint32_t))usmart_dev.funs[id].func)(temp[0], temp[1], temp[2], temp[3], temp[4]);
            break;

        case 6: /* 有6个参数 */
            res = (*(uint32_t(*)(uint32_t,uint32_t,uint32_t,uint32_t,uint32_t,uint32_t))usmart_dev.funs[id].func)(temp[0], temp[1], temp[2], temp[3], temp[4], \
                    temp[5]);
            break;

        case 7: /* 有7个参数 */
            res = (*(uint32_t(*)(uint32_t,uint32_t,uint32_t,uint32_t,uint32_t,uint32_t,uint32_t))usmart_dev.funs[id].func)(temp[0], temp[1], temp[2], temp[3], temp[4], \
                    temp[5], temp[6]);
            break;

        case 8: /* 有8个参数 */
            res = (*(uint32_t(*)(uint32_t,uint32_t,uint32_t,uint32_t,uint32_t,uint32_t,uint32_t,uint32_t))usmart_dev.funs[id].func)(temp[0], temp[1], temp[2], temp[3], temp[4], \
                    temp[5], temp[6], temp[7]);
            break;

        case 9: /* 有9个参数 */
            res = (*(uint32_t(*)(uint32_t,uint32_t,uint32_t,uint32_t,uint32_t,uint32_t,uint32_t,uint32_t,uint32_t))usmart_dev.funs[id].func)(temp[0], temp[1], temp[2], temp[3], temp[4], \
                    temp[5], temp[6], temp[7], temp[8]);
            break;

        case 10:/* 有10个参数 */
            res = (*(uint32_t(*)(uint32_t, uint32_t,uint32_t,uint32_t,uint32_t,uint32_t,uint32_t,uint32_t,uint32_t,uint32_t))usmart_dev.funs[id].func)(temp[0], temp[1], temp[2], temp[3], temp[4], \
